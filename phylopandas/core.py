...
pandas.core.frame.DataFrame = frame.DataFrame


# Functions to convert pandas `read` functions to phylopandas read functions.
def _pandas_to_phylopandas_converter(func):
    """Decorate to covert all Pandas `read_` functions to output
    phylopandas.DataFrame"""
    @wraps(func)
    def wrapper(*args, **kwargs):
        df = func(*args, **kwargs)
        return frame.DataFrame(df)
    return wrapper
...
def _convert_pandas_functions_to_phylopandas(pandas_module):
    ...
    for name in dir(pandas_module):
        ...
        if name[:5] == 'read_':
            ...
            setattr(pandas_module, name,
                    _pandas_to_phylopandas_converter(func))
